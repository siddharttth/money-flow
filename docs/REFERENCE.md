...
| Element | Calculation | Representation |
| --- | --- | --- |
| **Flow curve** | Cumulative running total per day. Solid = this month; dashed = last month at the same day index. Runs to **today** in a live month, not to the 31st. Needs ≥ 2 points. | SVG area + line, 230px minimum, growing to fill the card's height. A key names both months ("September — August, same days"). **When last month has no spending at all**, the dashed line is not drawn, the key says "No spending recorded in \<month\> to compare against", and the subtitle drops the comparison ("Spending added up, day by day"). |
| Reading the line | The running total on any day, and last month's on the same day | Mouse: hover anywhere. Touch: **drag sideways** along the chart — a tap does nothing and a vertical drag scrolls the page (`pan-y`). A guide rule follows; the readout swaps instantly, never animated. Desktop: a figure floating on the curve (date, ₹ this month, "Aug ₹X"). **Phone: a readout bar pinned above the chart** instead, resting on today. |
| Today's point | The last point of a live month | Breathes in the header's live-dot pulse (2.4s) — the one point still moving. Still in a past month and under reduced motion. Points are HTML, not SVG, so the stretched plot does not squash them. |
| Newest segment | Today's total changing while the page is open | Eases to its new value (220ms); the rest of the line never re-animates, and nothing moves on arrival. |
| Projection shading | From today to month end, under the run-out line | A faint fill (the accent when a what-if rate is being tried) — "not happened yet" before any figure says so |
...
**A balance reaching ₹0** gets its own moment instead (`useJustCleared`): a soft
green sweep across the row, not the accent wash, and the amount counting off
from what was owed to ₹0 over ~0.7s, then "Settled ✓ · balance cleared" for
~2s before the row goes back to "—". Only for a balance that was open while
you watched — never a row that loads settled. Reduced motion: no sweep, the
figure goes straight to "Settled ✓".
